...
use crate::xlsdk::entrypoint::excel_free;
use crate::xlsdk::xlcall::{xlbitDLLFree, xlbitXLFree, xlerrDiv0, xlerrGettingData, xlerrNA, xlerrName, xlerrNull, xlerrNum, xlerrRef, xlerrValue, xloper12__bindgen_ty_1, xloper12__bindgen_ty_1__bindgen_ty_1, xloper12__bindgen_ty_1__bindgen_ty_3, xltypeBool, xltypeErr, xltypeInt, xltypeMissing, xltypeMulti, xltypeNil, xltypeNum, xltypeRef, xltypeSRef, xltypeStr, LPXLOPER12, XLMREF12, XLOPER12, XLREF12, xloper12, xlref12, xlmref12, RW, COL, LPXLMREF12, LPXLREF12};
...
pub struct Variant(pub(crate) XLOPER12);
...
impl Variant {
    ...
    pub const fn const_missing() -> Variant {
        Variant(XLOPER12 {
            xltype: xltypeMissing,
            val: xloper12__bindgen_ty_1 { w: 0 },
        })
    ...
    pub fn as_xloper(&mut self) -> &XLOPER12 {
        &mut self.0
    }
...
/*impl From<*mut xlref12> for Variant {
    fn from(vv:*mut xlref12) -> Variant {
        let v = unsafe {*vv};
        Variant(XLOPER12 {
            xltype: xltypeSRef,
            val: xloper12__bindgen_ty_1 {
...
impl From<LPXLREF12> for Variant {
    fn from(xlref: LPXLREF12) -> Variant {
        let v = unsafe { &*xlref };
        let mut result = Variant::as_sref(v.rwFirst, v.rwLast, v.colFirst, v.colLast);
        result.0.xltype &= xltypeMask; // no ownership bits
        result
    }
}

impl From<LPXLMREF12> for Variant {
    fn from(xlref: LPXLMREF12) -> Variant {
        let v = unsafe { &*xlref };
        let mut result = Variant::as_sref(v.reftbl[0].rwFirst, v.reftbl[0].rwLast, v.reftbl[0].colFirst, v.reftbl[0].colLast);
        result.0.xltype &= xltypeMask; // no ownership bits
        result
    }
}
...
impl fmt::Display for Variant {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.0.xltype & xltypeMask {
            ...
            xltypeRef => {
                let sref = unsafe { (*self.0.val.mref.lpmref).reftbl[0] };
                write!(
                    f,
                    "Mref:({}) -> ({},{}) -> ({},{})",
                    unsafe { self.0.val.mref.idSheet as u32 },
                    sref.rwFirst, sref.colFirst,
                    sref.rwLast, sref.colLast
                )
            },
            v => write!(f, "#BAD_ERR {}", v),
        }
