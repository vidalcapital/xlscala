...
use crate::xlsdk::xlcall::{xlFree, xlretFailed, LPXLOPER12, XLOPER12};
use std::ffi::CStr;
use std::mem;
use std::ptr;
...
type EXCEL12PROC_0 = extern "stdcall" fn(
    xlfn: ::std::os::raw::c_int,
    count: ::std::os::raw::c_int,
    xloper12res: LPXLOPER12,
) -> ::std::os::raw::c_int;
...
/*pub fn excel12(xlfn: u32, opers: &mut [Variant]) -> Variant {
    ...
    match res {
        0 => result,
        v => {
            Logger::debug(&format!("ReturnCode {}", v));
            result
        }
    }
}*/
...
pub fn excel12v(xlfn: i32, oper_res: &mut XLOPER12, opers: &[LPXLOPER12]) -> i32 {
    ...
    unsafe {
        if PEXCEL12 == 0 {
            ...
            let p = opers.as_ptr();let len = opers.len();
            mem::transmute::<usize, EXCEL12PROC>(PEXCEL12)(xlfn, len as i32, p, oper_res)
        }
    }
}

pub fn excel12v_0(xlfn: i32, oper_res: &mut XLOPER12) -> i32 {
    fetch_excel12_entry_pt();
    ...
    unsafe {
        if PEXCEL12 == 0 {
            ...
            mem::transmute::<usize, EXCEL12PROC>(PEXCEL12)(xlfn, 0, core::ptr::null_mut(), oper_res)
        }
    }
}
